...
function isCommandDefinition(value: unknown): value is AnyCommandDefinition {
    if (!isRecord(value)) {
        return false;
    }
...
function isCommandGroup(value: unknown): value is CommandGroup {
    if (!isRecord(value)) {
        return false;
    }
...
function groupKey(segments: readonly string[]): string {
    return segments.join("/").toLowerCase();
}

function defaultGroupDescription(name: string): string {
    const readableName = name.replace(/[-_]+/gu, " ");

    return `${readableName[0]?.toUpperCase() ?? ""}${readableName.slice(1)} commands.`;
}

function createDefaultGroup(name: string): CommandGroup {
    return {
        description: defaultGroupDescription(name),
    };
}

function toCommandPath(segments: readonly string[]): CommandPath | null {
 ...
 */
export function resolveCommandModules(
    modules: readonly LoadResult[],
): CommandResolutionResponse {
    const response: CommandResolutionResponse = {
        routes: [],
        skipped: [],
        failed: [],
    };
    ...
    const commandModules: LoadResult[] = [];
    ...
    for (const loadedModule of commandModules) {
        ...
        if (!commandPath) {
            response.failed.push({
                file: loadedModule.relativePath,
                error:
                    `Command route "${routeSegments.join(" ")}" is too deep. ` +
        ...
        const routeGroups = loadedModule.directorySegments.map(
            (segment, index) => {
                const key = groupKey(
                    loadedModule.directorySegments.slice(0, index + 1),
                );
    ...
    response.routes.sort((left, right) =>
