...
const getCityPart = (timezone: string): string => {
    const parts = timezone.split("/");
    return parts[parts.length - 1] ?? timezone;
};

const chooseBestTimezoneName = (
...
const getAllowedTypoDistance = (inputLength: number): number => {
...
const buildSingleZoneCountryMap = (): Map<string, string> => {
...
const TIMEZONE_INDEX: TimezoneIndexEntry[] = moment.tz.names().map((name) => ({
    name,
    normalizedName: normalizeTimezoneInput(name),
    normalizedCity: normalizeTimezoneInput(getCityPart(name)),
}));

const TIMEZONE_BY_NORMALIZED_NAME = new Map<string, string>();
const TIMEZONES_BY_NORMALIZED_CITY = new Map<string, string[]>();
...
for (const timezone of TIMEZONE_INDEX) {
    ...
    const existingCityMatches =
        TIMEZONES_BY_NORMALIZED_CITY.get(timezone.normalizedCity) ?? [];

    existingCityMatches.push(timezone.name);
    TIMEZONES_BY_NORMALIZED_CITY.set(
        timezone.normalizedCity,
        existingCityMatches,
    );
}

export const resolveTimezone = (input: string): string | null => {
    ...
    const exactCityMatches =
        TIMEZONES_BY_NORMALIZED_CITY.get(normalizedInput) ?? [];
            ...
            if (typoDistance <= maxDistance) {
                const candidate: MatchCandidate = {
                    ...
                    score:
                        ...
                        typoDistance * 10 +
                        Math.abs(
                            timezone.normalizedCity.length -
