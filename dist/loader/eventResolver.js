 ...
 */
const DEPRECATED_CLIENT_EVENT_NAMES = ["ready", "webhookUpdate"];
...
function isClientEventName(value) {
    return typeof value === "string" && CLIENT_EVENT_NAMES.has(value);
}
function getEventCandidate(moduleExports) {
    return moduleExports.default ?? moduleExports.event;
}
function validateEventDefinition(candidate) {
    ...
    if (!isClientEventName(candidate.name)) {
        if (typeof candidate.name === "string") {
            return {
                success: false,
                error: `"${candidate.name}" is not a supported ` +
 ...
 */
export function resolveEventModules(modules) {
    const response = {
        handlers: [],
        skipped: [],
        failed: [],
    ...
    for (const loadedModule of modules) {
        ...
        if (candidate === undefined) {
            response.skipped.push({
                ...
                reason: "Module does not export an event as " +
                    'default or as "event".',
            });
            continue;
        }
    ...
    response.handlers.sort((left, right) => left.file.localeCompare(right.file));
    return response;
}
